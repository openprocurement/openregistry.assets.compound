...
from openregistry.assets.compound.tests.base import (
    test_asset_compound_data, test_asset_compound_data_341, BaseAssetWebTest
)
...
class AssetCompoundResourceTest(BaseAssetWebTest, ResourceTestMixin, AssetResourceTestMixin):
    ...
    def test_create_compount_with_item_schemas_341(self):
        response = self.app.post_json('/', {'data': test_asset_compound_data_341})
        self.assertEqual(response.status, '201 Created')
        self.assertEqual(response.content_type, 'application/json')
        response = response.json['data']
        self.assertEqual(response['items'][0]['schema_properties']['properties'], test_asset_compound_data_341['items'][0]['schema_properties']['properties'])
        self.assertEqual(response['items'][0]['schema_properties']['code'][0:2], test_asset_compound_data_341['items'][0]['schema_properties']['code'][:2])
        self.assertEqual(response['items'][0]['description'], test_asset_compound_data_341['items'][0]['description'])
        self.assertEqual(response['items'][0]['classification'], test_asset_compound_data_341['items'][0]['classification'])
        self.assertEqual(response['items'][0]['additionalClassifications'], test_asset_compound_data_341['items'][0]['additionalClassifications'])
        self.assertEqual(response['items'][0]['address'], test_asset_compound_data_341['items'][0]['address'])
        self.assertEqual(response['items'][0]['id'], test_asset_compound_data_341['items'][0]['id'])
        self.assertEqual(response['items'][0]['unit'], test_asset_compound_data_341['items'][0]['unit'])
        self.assertEqual(response['items'][0]['quantity'], test_asset_compound_data_341['items'][0]['quantity'])
    ...
    def test_bad_item_schemas_code_341(self):
        bad_initial_data = deepcopy(test_asset_compound_data_341)
        bad_initial_data['items'][0]['classification']['id'] = "42124210-6"
        response = self.app.post_json('/', {'data': bad_initial_data},status=422)
        self.assertEqual(response.status, '422 Unprocessable Entity')
        self.assertEqual(response.content_type, 'application/json')
    ...
    def test_delete_item_schema_341(self):
        response = self.app.post_json('/', {'data': test_asset_compound_data_341})
        self.assertEqual(response.status, '201 Created')
        resource = response.json['data']
        self.resource_token = response.json['access']['token']
